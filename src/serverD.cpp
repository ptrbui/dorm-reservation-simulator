...
int main() {
    ...
    // Load double room information for serverD from double.txt
    std::string filename = "../data/double.txt";
    ...
    if ((rv = getaddrinfo(LOCALHOST, SERVER_D_PORT, &hints, &servinfo)) != 0) {
    ...
    std::cout << "The Server D is up and running using UDP on port " << SERVER_D_PORT << std::endl;
